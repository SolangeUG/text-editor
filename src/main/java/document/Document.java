...
public abstract class Document {
	 ...
	 * The reason we put countSyllables here because we'll use it again next week
	 * when we implement the EfficientDocument class.
	 * 
	 * For reasons of efficiency you should not create Matcher or Pattern objects inside this method.
	 * Just use a loop to loop through the characters in the string and
	...
	int countSyllables(String word) {
		...
		// DONE: Implement this method so that you can call it from the
		...
		for (int i = 0; i < cArray.length; i++) {
			...
			int ndx = vowels.indexOf(Character.toLowerCase(cArray[i]));
			if (newSyllable && ndx >= 0) {
				newSyllable = false;
				numSyllables++;
			} else if (ndx < 0) {
				newSyllable = true;
	 ...
	 */
	static boolean testCase(Document doc, int syllables, int words, int sentences) {
	...
	public double getFleschScore() {
	    // DONE: Play with this method in week 1, and then implement it in week 2
